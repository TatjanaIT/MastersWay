...
def create_schema(cur):
    ...
    '''
def create_types(cur):
    cur.execute(f"""
    DO $$
    BEGIN
        IF to_regtype('"{settings.SCHEMA}".pricing_plan_type') IS NULL THEN
            CREATE TYPE "{settings.SCHEMA}".pricing_plan_type
            AS ENUM ('free', 'ai-starter','starter', 'pro', 'b2b');
        END IF;
    END $$;
    """)
...
def create_tables(cur):
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS composite_ways(
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS former_mentors_ways(
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS mentor_users_ways(
        "user_uuid" UUID NOT NULL REFERENCES users("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
        "way_uuid" UUID NOT NULL REFERENCES ways("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS favorite_users_ways(
        "user_uuid" UUID NOT NULL REFERENCES users("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
        "way_uuid" UUID NOT NULL REFERENCES ways("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS to_user_mentoring_requests(
        "user_uuid" UUID NOT NULL REFERENCES users("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
        "way_uuid" UUID NOT NULL REFERENCES ways("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS from_user_mentoring_requests(
        "user_uuid" UUID NOT NULL REFERENCES users("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
        "way_uuid" UUID NOT NULL REFERENCES ways("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS "plans_job_tags"(
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS "job_dones"(
        "uuid" UUID NOT NULL DEFAULT (uuid_generate_v4()),
        "created_at" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "updated_at" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "description" VARCHAR(3000) NOT NULL,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS "job_dones_job_tags"(
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS "way_collections"(
        "uuid" UUID NOT NULL DEFAULT (uuid_generate_v4()),
        "owner_uuid" UUID NOT NULL REFERENCES users("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
        "created_at" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "updated_at" TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ...
    cur.execute("""
    CREATE TABLE IF NOT EXISTS "way_collections_ways"(
    ...
    """)
    cur.execute("""CREATE UNIQUE INDEX IF NOT EXISTS "way_tag_name_key" ON "way_tags"("name");""")

    cur.execute("""
    CREATE TABLE IF NOT EXISTS "ways_way_tags"(
        "way_uuid" UUID NOT NULL REFERENCES ways("uuid") ON UPDATE CASCADE ON DELETE CASCADE,
    ...
    cur.execute("""
    CREATE UNIQUE INDEX IF NOT EXISTS "user_tag_name_key"
    ON "user_tags"("name");
    """)
    ...
    CREATE TABLE IF NOT EXISTS "profile_settings"(
        "uuid" UUID NOT NULL DEFAULT uuid_generate_v4(),
        "pricing_plan" "{settings.SCHEMA}".pricing_plan_type NOT NULL,
...
def create_functions_and_triggers(cur):
    # --- init_user + trigger ---
    cur.execute("""
    ...
    # max likes given to users
    cur.execute("""
    CREATE OR REPLACE FUNCTION check_max_likes_to_user()
    RETURNS TRIGGER AS $$
    BEGIN
    ...
    # --- max likes received from users ---
    cur.execute("""
    CREATE OR REPLACE FUNCTION check_max_likes_from_user()
    RETURNS TRIGGER AS $$
    BEGIN
    ...
    # --- max mentoring requests from way to mentor (for mentor) ---
    cur.execute("""
    ...
    # --- max mentoring requests from one user to many ways ---
    cur.execute("""
    ...
    # max req from way to mentor for way
    cur.execute("""
    ...
    # max req from mentor to way for way
    cur.execute("""
    ...
    # max problems in report ---
    cur.execute("""
    CREATE OR REPLACE FUNCTION check_max_problems_in_report()
    RETURNS TRIGGER AS $$
    BEGIN
    ...
    cur.execute("""
    DROP TRIGGER IF EXISTS max_problems_in_report_trigger ON problems;
    CREATE TRIGGER max_problems_in_report_trigger
    BEFORE INSERT ON problems
    FOR EACH ROW
    EXECUTE FUNCTION check_max_problems_in_report();
    """)

    # max comments in report
    cur.execute("""
    ...
    cur.execute("""
    DROP TRIGGER IF EXISTS check_max_users_trigger ON users_projects;
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            ...
            create_types(cur)
            create_tables(cur)
            create_functions_and_triggers(cur)
        conn.commit()
        print(f"Готово: schema={settings.SCHEMA} создана, таблицы созданы, триггеры обновлены.")
    finally:
        conn.close()
